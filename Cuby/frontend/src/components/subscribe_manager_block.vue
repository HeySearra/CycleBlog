<template>
    <div style="padding:10px 30px" :class="list.length==0?'':'bottom0'">
        <div>
            ...
            <div v-if="!list.length&&!loading" class="not_found">你还没有任何关注的人</div>
            <div v-if="loading" class="not_found">加载中 <i class="el-icon-loading"></i></div>
            <div v-for="item in list" :key="item" class="margin0">
                <ulb type="subscribe" :uid="item" ref="ulb" @done="loading_done_msg"></ulb>
...
export default {
    ...
    methods:{
        ...
        apply_for_info(){
            ...
            $.ajax({ 
                type:'get', 
                url:"/create/follow?page="+that.current_page+'&each='+that.page_size,
